function [TS, TR, TT, ER, t] = ssicp(q,p,varargin)
...
for k=1:arg.iter
    ...
    switch arg.Minimize
        case 'point'
            ...
            %else
                [S,R,T] = eq_point_scale(q(:,q_idx),pt(:,p_idx), weights(p_idx), R1);
            %end
        case 'plane'
            weights = arg.Weight(match);
    ...
    TR(:,:,k+1) = R*TR(:,:,k);
    TS(:,:,k+1) = S*TS(:,:,k);
    TT(:,:,k+1) = S*R*TT(:,:,k)+T; % Lu
    R1= R;
    
    % Apply last transformation
    pt = TS(:,:,k+1) * TR(:,:,k+1) * p + repmat(TT(:,:,k+1), 1, Np);
...
function [S,R,T] = eq_point_scale(q,p,weights, R1)
...
q_mark = q - repmat(q_bar, 1, n);
% Apply weights
q_mark_weight = q_mark .* repmat(weights, 3, 1);
...
% S = eye(3);
% for i = 1:3
%     S(i,i) = R1(i,:)*q_mark*p_mark(i,:)'/(p_mark(i,:)*p_mark(i,:)');
% end
S = sum(sum(R1*q_mark.*p_mark))/sum(sum(p_mark.*p_mark));
...
N = S*p_mark*transpose(q_mark_weight); % taking points of q in matched order
...
%循环遍历每个点，对于每个点，它从其最近邻点集合中计算一个协方差矩阵
%然后通过特征值分解找到该协方差矩阵的最小特征值对应的特征向量，作为法向量。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
